...
（3）注意子进程判断完毕务必调用exit(0)退出，否则系统有死机风险。

说明：因为如果子进程不主动退出，那么子进程内的循环继续，而每一次循环都会创建子进程。也即第一个子进程会接着创建199个子子进程，第二个子进程会创建198个子子进程，以此类推，子进程数量成了200!这么多个，系统直接资源不足死机。
...
**说明：调用`exit()`和`_exit()`之间的区别**？

`exit()`函数不仅终止调用它的进程，还会进行一些清理工作：

- 需要包含`stdlib.h`头文件
- 它调用所有通过`atexit`注册的清理函数；
- 它会刷新所有的输出缓冲区；
- 它会关闭所有通过`fopen`打开的文件流；
- 它适用于在退出前需要进行资源清理的场景；

`_exit()`函数立即终止调用它的进程，不进行任何清理工作：

- 需要包含`unistd.h`头文件；
- 适用于需要立即退出且不需要进行额外清理工作的情况；
- 在某些低级系统编程或者异常处理场景中，可能会使用它来避免不必要的开销；
